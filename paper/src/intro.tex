...
\todo[inline]{Truong: I think we should do away with the term ``data predictive control''.}
...
\todo[inline]{You need to be very careful about the above statement. It can be difficult for buildings, but as I know, MPC is very popular and widely used in process control. All industrial control systems like Siemens PLC, ABB have software and libraries and great support for MPC for these control applications.  Your net catches too wide, it may backfire.}
...
We solve this problem with \textit{Data Predictive Control (DPC)}\todo{Again, remove DPC.} by bridging the gap between Machine Learning and Predictive Control.
...
\todo[inline]{Use affirmative and stronger words, like we do this, we achieve that, rather than we attempt to answer this and do that (which sound very weak, close to failure).}
