...
we loop through the full data set \(\D\) to identify which sample maximizes the information gain defined in \eqref{E:ig:final}. In this setup, we solve the following optimization problem
\begin{align}
\label{E:oed:batch}
...
\end{align}
\todo[inline]{The above equation needs to be fixed. Either use the final equation as in the cited paper, or write the general $\text{IG}(\dots)$.}
...
\begin{figure*}[t]
	...
	\input{figures/batch-oed.tex} \hspace{0.5cm}
	\input{figures/batch-rand.tex}
	\caption{Left: Optimal subset of data selection. Right: Selection using random sampling. The mean prediction error and prediction variance are both lower for the optimal selection done using Algo.~\ref{A:oed:batch}.}
