...
We use two different DoE Commercial Reference Building (DoE CRB) simulated in EnergyPlus as the virtual test-bed building.
...
\textit{Weather variables \(d^w\):} outside temperature and humidity - these features are defined in EnergyPlus.
...
\textit{Control variables \(u\):} cooling, supply air temperature and chilled water setpoints - these will be optimized in the MPC problem for Power Tracking Reference Control in Sec.~\ref{SS:power_tracking}.

\textit{Output variable \(y\):} total power consumption - this is the output of interest which we will predict using all the above features in the GP model.
...
We learn a single GP model and use the \emph{zero-variance method} to predict the outputs \(y\) at the following time steps. For each prediction step $t+\tau$, \( \tau \in \{0,\dots,N-1\}\), the output \(y_{t+\tau} | x_{t+\tau}\) is a Gaussian random variable given by:
\begin{gather}
\label{E:gp:casestudy}
y_{t+\tau} | x_{t+\tau} \sim \GaussianDist{\bar{y}_{t+\tau}}{\sigma^2_{y,t+\tau}}, \\
x_{t}\!=\![\bar y_{t-l}, \dots, \bar y_{t-1}, u_{t-m}, \dots, u_t, w_{t-p}, \dots, w_{t-1}, w_t], \nonumber
\end{gather}
...
\end{gather}
Here, \(D\) is the dimension of \(x\), \(k_3(x,x')\) is applied to only nontemporal features like time of day and day of week, while \(k_1(x,x')\) and \(k_2(x,x')\) are applied to all the remaining features as proposed in \cite{nghiemetal16gp}. We optimize the parameters \(\theta = [\mu, k, \sigma_{f_2}, \lambda_d, \sigma_{f_3}, \alpha, \lambda] \) of the model in \eqref{E:gp:casestudy} using GPML \cite{Rasmussen2010}. After training, the less important features, i.e.~features with high \(\lambda_d\) are removed and the models are trained again. 
...
For OED, we only require the first model with \(\tau=0\) in \eqref{E:gp:casestudy}. We simplify the notation by denoting \(t|t\) by \(t\).
Now, the goal at time \(t\) is to determine what should be the optimal cooling \(u_{\mathrm{clg},t}\), supply air temperature \(u_{\mathrm{sat},t}\), and chilled water temperature \(u_{\mathrm{chw},t}\) setpoints which when applied to the building will require power consumption \(y_t\) such that \((x_t,y_t)\) can be used to learn \(\theta\) as efficiently as possible.
...
In practice, the chilled water temperature cannot be changed by more than \(0.15^o\mathrm{C/min}\). Keeping this and thermal comfort into consideration, we consider the following operation constraints:
\begin{gather}
...
For the hotel building on the left, information gain is the best in terms of RMSE and SMSE.
Uniform random sampling and PRBS are far worse in both metrics for first \(7-10\) days (\(\sim200\) hrs), beyond which we obtain sufficient data that it is hard to distinguish between the performance and model accuracies.
...
It is observed that OED can be used to learn a model vary fast. However, when random sampling can be done for sufficiently long time, the two approaches result in similar models. Due to operation constraints, often, function tests cannot be done for sufficiently long time, in which case even few hours of periodic OED can provide far better models due to its ability to capture more information.
...
A controller computes the setpoints for the building and the power of the battery to optimally track the reference demand signal.
...
The office building has a larger HVAC system, so for this building we consider the following Demand Response scenario. 
