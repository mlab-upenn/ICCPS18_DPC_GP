...
%
% The GP model is updated with new data on-the-fly.
%
...
classdef DRtrackingWithStorage_SOCP_singlegp_evolving < OBNNode
    properties (Access=public)
        ...
        evolving_data;     % All data related to evolving model
    end
    
    methods (Access=public)
        function obj = DRtrackingWithStorage_SOCP_singlegp_evolving(gpmodels, normparams, ref, forecasts, active_ahead, horizon, ...
                wdelta, wPb, wvar,...
                ramplimit, ...
            ...
            %
            % evolving_params is a structure of the parameters for evolving
            % GP:
            %   .period = the number of time steps when model updating is
            %               performed.
            %   .method = method to select the training points, one of 'IG'
            %               for Information Gain.
            %   .maxpoints = the maximum number of points in the training
            %                set of the GP.
            %
            ...
            % Check evolving GP params
            assert(isstruct(evolving_params) && ...
                all(isfield(evolving_params, {'period', 'method', 'maxpoints'})) && ...
                evolving_params.period >= 1 && ischar(evolving_params.method) && evolving_params.maxpoints > 1, ...
                'Invalid evolving GP parameters.');
            ...
            obj.evolving_data = evolving_params;
            obj.evolving_data.last = 0;     % the last step when the GP was updated
        ...
        function calculateDRsignal(self)            
            ...
            % ===== Evolving GP =====
            if self.curStep - self.evolving_data.last > self.evolving_data.period
                % Update the GP model with new data since last time
                fprintf('Updating the GP model with new data in the last %d time steps...\n', self.evolving_data.period);
                self.update_evolve_gp(self.evolving_data.last+1, self.curStep-1);

                % update the evolving GP status
                self.evolving_data.last = self.curStep-1;
            end
            % ===== Evolving GP =====
...
%                end
    ...
    methods (Access=private)    
        ...
        function update_evolve_gp(self, tbegin, tend)
            % Update the GP model with data between time steps tbegin, tend
            % (inclusively).

            % First get the new data in input forms for the GP
            [xnew, ynew] = self.gpmodels.signalmodel.getIOVectors(tbegin:tend, 'removeNaNs', true);
            
            % Concatenate it with the existing training data as candidates
            data = self.gpmodels.gp.training_data;
            data.x = [data.x; xnew];
            data.y = [data.y; ynew];
            
            if length(data.y) > self.evolving_data.maxpoints
                % Only when we have more data than allowed, we need to
            ...
            self.gpmodels.gpdata.setTrainingData(data.x, data.y);
        end
