...
%                end
            ...
            [LgtSP, ClgSP, SupplyAirSP, ChwSP] = self.defaultSetpoints(tod, dayofweek);
            
            % Save uncontrolled setpoints
            self.signals.setSignal('LgtSP', self.curStep, LgtSP);
            ...
            if self.last_active
                ...
                options = struct('quiet', true, ...
                                 'ipopt', struct('linear_solver', 'ma57', 'tol', 1e-4, 'acceptable_tol', 1e-3, 'max_iter', 300, 'print_level', 0));
                ...
                % denormalized values
                self.signals.setSignalUnnormalized('ClgSP', self.curStep, self.prevSol.ClgSP(1));
                ClgSP = self.signals.getSignalDenorm('ClgSP', self.curStep);
            ...
            % Send to building
            self.output('setpoints', [ClgSP, LgtSP, SupplyAirSP, ChwSP]);
    ...
    methods (Static)
        ...
        function [LgtSP, ClgSP, SupplyAirSP, ChwSP] = defaultSetpoints(tod, dow)
            tod = rem(tod, 24);
            ...
            elseif dow == 7
                ...
                daytype = 2;    % all other days (sunday)
            end
    ...
    methods (Access=private)    
        ...
        function optvar = create_optstructs(self, horizon, nPbVars, X0)
            ...
            optvar.ClgSP = optivar(horizon, 1, 'ClgSP');
            setLb(optvar.ClgSP, -1);
            setUb(optvar.ClgSP, 1);
        ...
        function inputmatrix = construct_data(self, todseries, dowseries, spseries, k, optvar)
            ...
            assert(~any(isnan(todseries)) && ~any(isnan(dowseries)) && ~any(isnan(spseries(:,1))));
            ...
            inputsubs.ClgSP.start = self.curStep;
            inputsubs.ClgSP.values = optvar.ClgSP;
            ...
            self.signals.setSignal('LgtSP', ts, spseries(:,1));
        ...
        function G = simulate_naive(self, optvar, curHour, curDay)
            ...
            spseries = zeros(self.curHorizon, 4);
            for kk = 1:self.curHorizon
                [spseries(kk,1), spseries(kk,2), spseries(kk,3), spseries(kk,4)] = ...
                    self.defaultSetpoints(todseries(kk), dowseries(kk));
            end
