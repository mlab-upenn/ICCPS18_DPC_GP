...
%                       called as
%
%                         y_star = problem.f(x_star, idx)
%                       where idx is the index of the x_star in
%                       candidate_x_star.
%
...
function results = learn_gp_hyperparameters_evolve(problem, model, varargin)

  % parse optional inputs
  parser = inputParser;

  addParameter(parser, 'num_restarts', 0);
  addParameter(parser, 'minFunc_options', ...
                        struct('Display',     'off', ...
                               'MaxFunEvals', 100));
  addParameter(parser, 'retrain_maxskip', 1);
  ...
  for i = 1:problem.num_evaluations
    ...
    if kSkip <= 0
        ...
        kSkip = curSkip;
        if curSkip < options.retrain_maxskip
            curSkip = curSkip + 1;
        end
    else
        results.map_hyperparameters(i) = initial_hyperparameters;
    end
    kSkip = kSkip - 1;
    ...
    x = [x; x_star];
    y = [y; problem.f(x_star, ind)];

    problem.candidate_x_star = ...
        problem.candidate_x_star([1:(ind - 1), (ind + 1):end], :);
    
    if has_dispstat
        dispstat(sprintf('Progress: have selected %d / %d points (%03d%%).', i, problem.num_evaluations, ...
            fix(i*100/problem.num_evaluations)));
    end
  end
