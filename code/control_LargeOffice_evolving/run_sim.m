...
BASELINE = load(fullfile('..', 'data', 'sim-nominal-LargeOffice.mat'));

% Calculate the first time step
if exist('StartMonth', 'var') && exist('StartDay', 'var')
    startIdx = find(BASELINE.Month == StartMonth & BASELINE.DOM == StartDay, 1);
    assert(~isempty(startIdx));
    
    if startIdx > 1 && BASELINE.TOD(startIdx) > 0 && BASELINE.TOD(startIdx) < 24
        % Start from 00:00 on that day
        startIdx = startIdx - 1;
    end
    
    BASELINE = slice_data(BASELINE, startIdx);
end
...
if multipleGPs
    ...
    RESULTS = load(fullfile('..', 'models', 'LargeOffice', control_gp_model_file));
...
try
    b = runSimulation(controller, 600, false);
catch ME
...
if b
    ...
    if isprop(controller, 'signals')
        ...
        RESULTS.calc_LgtSP = signals_hist.LgtSP;
