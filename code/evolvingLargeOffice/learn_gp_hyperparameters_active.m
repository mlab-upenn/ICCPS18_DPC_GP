...
function results = learn_gp_hyperparameters_active(problem, model, savedata, varargin)
  ...
  for i = 1:problem.num_evaluations
    ...
    if rem(i,96)==0
        n_days_init = savedata.n_days_init;
        shift = savedata.shift;
        n_days_next = savedata.n_days_next;
        n_days = n_days+1;
        X_chosen_active = x(1:end-1,:);
        y_chosen_active = y(1:end-1);
        saveStr = sprintf('evolving_%dinit_%dshift_%dupdate_%dday_%04d%02d%02d_%02d%02d.mat',...
            n_days_init, shift, n_days_next, n_days, YY, MM, DD, HH, MINS);
        save(fullfile('data', saveStr), 'X_chosen_active', 'y_chosen_active');
