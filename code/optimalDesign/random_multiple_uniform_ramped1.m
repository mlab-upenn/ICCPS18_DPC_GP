...
switch sample_type
    ...
    case 'prbs'
        ...
        ChwSPrand = idinput(MAXSTEPS,'prbs')';
end
...
while kStep <= MAXSTEPS    
    ...
    if kStep <= order_autoreg
        ...
        % select random point within constraints
        ChwPrev = inputs(8,kStep-1);
        switch sample_type
            case 'uniform'
                ChwSP = max(ChwMin,ChwPrev-rampVal)+(min(ChwMax,ChwPrev+rampVal)-max(ChwMin,ChwPrev-rampVal))*rand;
            case 'prbs'
                ChwSP = ChwPrev+rampVal*ChwSPrand(kStep);
        end
...
saveStr = sprintf('random_sampling_%s_%dramped_%dinput_%dday_%04d%02d%02d_%02d%02d.mat',...
    sample_type, rampVal, numel(ctrl_vars), SimDays, YY, MM, DD, HH, MINS);
