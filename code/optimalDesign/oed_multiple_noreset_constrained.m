...
ChwMin = 3.7;
ChwMax = 9.7;
ctrl_vars_all_before7 = struct('ClgSP', linspace(ClgMin,ClgMax,n_steps),...
...
ctrl_vars_all_after7 = struct('ClgSP', linspace(ClgMin,ClgMax,n_steps),...
                       'KitchenClgSP', linspace(KitchenClgMin,KitchenClgMax,n_steps),...
                       'GuestClgSP', linspace(GuestClgMin+1,GuestClgMax-1,n_steps),...
                       'SupplyAirSP', linspace(SupplyAirMin+0.5,SupplyAirMax-0.5,n_steps),...
                       'ChwSP', linspace(ChwMin+1.5,ChwMax-1.5,n_steps));                   
...
if numel(ctrl_vars)==1
    X_c_star_before7 = eval(['ctrl_vars_all_before7.' ctrl_vars{1}])';
    X_c_star_after7 = eval(['ctrl_vars_all_after7.' ctrl_vars{1}])';
else
    [X_grid, Y_grid, Z_grid] = ndgrid(eval(['ctrl_vars_all_before7.' ctrl_vars{1}]), ...
                                  eval(['ctrl_vars_all_before7.' ctrl_vars{2}]), ...
                                  eval(['ctrl_vars_all_before7.' ctrl_vars{3}]));
    X_star = X_grid(:);
    Y_star = Y_grid(:);
    Z_star = Z_grid(:);
    X_c_star_before7 = [X_star, Y_star, Z_star];
    
    [X_grid, Y_grid, Z_grid] = ndgrid(eval(['ctrl_vars_all_after7.' ctrl_vars{1}]), ...
                                  eval(['ctrl_vars_all_after7.' ctrl_vars{2}]), ...
                                  eval(['ctrl_vars_all_after7.' ctrl_vars{3}]));
    X_star = X_grid(:);
    Y_star = Y_grid(:);
    Z_star = Z_grid(:);
    X_c_star_after7 = [X_star, Y_star, Z_star];
end
...
while kStep <= MAXSTEPS    
    ...
    if kStep <= order_autoreg
        ...
        % restricted testing after 7 am
        if dayTime <= 7*3600
            X_c_star = X_c_star_before7;
        else
            X_c_star = X_c_star_after7;
        end
...
saveStr = sprintf('doe_sampling_noreset_constrained_%s_%dinput_%dday_%04d%02d%02d_%02d%02d.mat',...
