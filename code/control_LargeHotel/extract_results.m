...
assert(~isempty(GuestClgSPVar), 'Guest_CLGSETP_SCH is missing.');

KitchenClgSPVar = findEPResultVar('KITCHEN_CLGSETP_SCH', 'Schedule Value', allObjs, allVars);
assert(~isempty(KitchenClgSPVar), 'Kitchen_CLGSETP_SCH is missing.');
...
[~, DATA, TS] = mlepLoadEPResults(...
    ...
    AMBIENTVar, HumidVar,...
    DoWVar, HolidayVar,...
    ChwSPVar, ClgSPVar, GuestClgSPVar, KitchenClgSPVar, SupplyAirSPVar,...
    PMVVars]);
...
RESULTS.ChwSP = DATA(:, 8);
RESULTS.ClgSP = DATA(:, 9);
RESULTS.GuestClgSP = DATA(:, 10);
RESULTS.KitchenClgSP = DATA(:, 11);
